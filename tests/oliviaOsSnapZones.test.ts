...
describe("resolveSnapBounds", () => {
  ...
  it("maximized는 전체 작업영역을 채운다", () => {
    const bounds = resolveSnapBounds("maximized", VW, VH, dockSafeArea);
    expect(bounds).toEqual({ x: 12, y: 8, width: VW - 24, height: VH - dockSafeArea - 8 });
  });

  it("left-half/right-half는 정확히 절반씩 나눈다", () => {
    const left = resolveSnapBounds("left-half", VW, VH, dockSafeArea);
    const right = resolveSnapBounds("right-half", VW, VH, dockSafeArea);
  ...
  it("left-70은 right-30보다 넓다", () => {
    const left70 = resolveSnapBounds("left-70", VW, VH, dockSafeArea);
    const right30 = resolveSnapBounds("right-30", VW, VH, dockSafeArea);
    expect(left70.width).toBeGreaterThan(right30.width);
  });

  it("right-70의 오른쪽 끝은 화면 오른쪽 여백과 맞는다", () => {
    const bounds = resolveSnapBounds("right-70", VW, VH, dockSafeArea);
