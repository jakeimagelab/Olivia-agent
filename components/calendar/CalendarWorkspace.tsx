  ...
  return (
    <div style={{ background: C.surface, border: `1.5px solid ${C.teal}`,
      ...
      <input ref={titleRef} value={title} onChange={e => setTitle(e.target.value)}
        ...
        style={{ fontSize: mfz(14, isMobile), fontWeight: mfw(700, isMobile), border: "none", outline: "none",
...
function EventDetailView({ task, onEdit, onToggle, isMobile = false }: { task: CalTask; onEdit: () => void; onToggle: () => void; isMobile?: boolean }) {
  ...
  const shareCard = async () => {
    ...
    try {
      ...
      await Promise.all(images.map((img) => (img.complete ? Promise.resolve() : img.decode().catch((error) => { console.error("[OLIVIA] Suppressed promise rejection", error); }))));
...
export default function CalendarWorkspace() {
  // OLIVIA OS Desktop의 AppWindow 안에 마운트되면 CalendarEmbedProvider가 true를 준다 —
  // (client-hub)의 고객관리 페이지와 달리 이 페이지는 GlobalHeader를 직접 그리므로, 창
  // 안에서는 그 호출만 건너뛴다. Provider 없이 렌더되는 기존 /calendar 방문은 기본값 false라
  // 전혀 바뀌지 않는다.
  const embedded = useCalendarEmbedded();
