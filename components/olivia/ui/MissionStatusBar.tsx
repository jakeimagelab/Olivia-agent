...
export default function MissionStatusBar({ title, status, currentStage, nextScheduleLabel, owner, progress, detailHref, contextNotice, onDetailClick, loading }: MissionStatusBarProps) {
  if (loading) {
    return (
  ...
  return (
    <div className="olivia-mission-bar">
      <div className="olivia-mission-bar__title">
        ...
        <strong>{title}</strong>
        {contextNotice ? <span className="olivia-mission-bar__context-notice">{contextNotice}</span> : null}
      ...
      {detailHref ? (
        <Link href={detailHref} className="olivia-mission-bar__detail" onClick={onDetailClick}>
