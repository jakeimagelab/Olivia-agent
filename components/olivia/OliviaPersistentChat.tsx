...
// 코드 요청서 — Olivia UX/Motion Core(2026-08-16), 이후 In-page Split Agent(2026-08-24)로
// 팝업을 인페이지 패널로 전환. 홈 이외의 페이지에서는 이 컴포넌트가 전역 In-page Agent
// 진입점으로 동작한다. 홈(/admin/dashboard/home)은 자체 큰 임베드 채팅(useOliviaLayoutStore)을
// 쓰므로 제외하고, client-portal은 직원용이 아닌 고객 대상 별도 화면이라 범위 밖이다 —
// 두 표면 다 같은 useOliviaConversationStore.messages를 구독하므로 대화 내용은 계속 같다.
// /photoclinic(QuoteBuilder 단독 페이지)은 예전엔 자체 임베드 팝업 채팅(OliviaChat)을 따로
// 썼지만, 그 팝업을 제거하고 여기 전역 In-page 패널로 통합했다 — 더 이상 예외가 아니다.
const localContextPages = ["/client-portal", "/admin/dashboard/home"];
