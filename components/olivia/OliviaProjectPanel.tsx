...
export default function OliviaProjectPanel({ workflowRunId }: { workflowRunId?: string | null }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/olivia/insights?workflowRunId=${encodeURIComponent(workflowRunId)}&limit=5`, { cache: "no-store" }).then((r) => r.json()).then((json) => { if (json.ok) setItems(json.insights ?? []); }).catch((error) => { console.error("[OLIVIA] Suppressed promise rejection", error); });
  }, [workflowRunId]);
