...
function buildContractHtml(q: QuoteData, signatureDataUrl = "", brand: ContractBrand = "photoclinic"): string {
  ...
  // deliveryTerms/paymentTerms는 채팅에서 "납품은 촬영 후 14일로"/"잔금은 촬영 전날" 같은 짧은
  // 조건 문구만 받아 고정 문장 구조에 끼워 넣는다(금액/기존 계좌·세금계산서 안내 문장은 항상
  // 코드가 결정론적으로 채운다 — LLM이 금액을 새로 짓지 않는다는 원칙, 스펙 §12).
  ...
  // specialTerms가 있으면(채팅 "특약에 ~~ 추가해줘"/"특약 빼") 그 내용이 메모 표시를 완전히
  // 대체한다 — update_quote_note와 같은 "전체 텍스트를 다시 씀" 원칙(부분 병합 안 함).
