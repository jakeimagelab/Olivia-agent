...
function buildSuggestions(data: ReturnType<typeof useHomeDashboardData>["data"]): Suggestion[] {
  ...
  if (list.length < 4) {
    ...
    if (list.length < 4) list.push({ id: "browse-clients", text: "진행 중인 프로젝트 현황을 살펴보세요.", href: "/clients", icon: FolderSearch });
    if (list.length < 4) list.push({ id: "browse-calendar", text: "이번 주 일정을 캘린더에서 정리해보세요.", href: "/calendar", icon: CalendarCheck2 });
    if (list.length < 4) list.push({ id: "checklist", text: "촬영 체크리스트를 확인해보세요.", href: "/clients", icon: ClipboardList });
  }
