...
export const OLIVIA_V2_TOOLS: FunctionTool[] = [
  ...
  { type: "function", name: "list_temporary_documents", description: "[READ] 고객과 아직 연결되지 않은 임시문서를 실제 DB에서 찾습니다. '예전에 만든 OO병원 문서', '임시문서 보여줘', 'OO병원 고객등록' 요청에서 먼저 호출해 정확한 temporaryDocumentId를 얻습니다.", strict: true, parameters: { type: "object", additionalProperties: false, properties: { hospitalName: { type: ["string", "null"] }, query: { type: ["string", "null"] }, limit: { type: ["number", "null"] } }, required: ["hospitalName", "query", "limit"] } },
  { type: "function", name: "approve_temporary_document", description: "[WRITE] 사용자가 이미지로 확인한 임시문서 내용에 첫 번째로 오케이/승인했을 때 상태를 고객등록 대기로 바꾸고 '고객으로 등록할까요?'라고 묻습니다. 이 단계에서는 고객을 만들지 않습니다.", strict: true, parameters: { type: "object", additionalProperties: false, properties: { temporaryDocumentId: { type: "string" } }, required: ["temporaryDocumentId"] } },
  { type: "function", name: "defer_temporary_document", description: "[WRITE] 사용자가 임시문서의 고객등록이나 내용 승인을 보류할 때 원본을 삭제하지 않고 임시문서함에 유지합니다.", strict: true, parameters: { type: "object", additionalProperties: false, properties: { temporaryDocumentId: { type: "string" } }, required: ["temporaryDocumentId"] } },
  { type: "function", name: "link_temporary_document_client", description: "[WRITE] 고객등록 질문에 사용자가 두 번째로 명시적 승인했을 때만 호출합니다. 병원 고객과 workflow를 생성 또는 재사용하고 같은 병원명의 열린 임시문서를 모두 연결합니다.", strict: true, parameters: { type: "object", additionalProperties: false, properties: { temporaryDocumentId: { type: "string" } }, required: ["temporaryDocumentId"] } },
