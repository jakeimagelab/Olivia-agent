...
export const OLIVIA_V2_TOOLS: FunctionTool[] = [
  ...
  { type: "function", name: "link_document_to_client", description: "[WRITE] 고객 등록·프로젝트 생성 전에 미리 만들어 저장한 견적서/계약서/콘티처럼, 병원명 오타 등으로 고객과 연결되지 않은 자료를 실제 고객에 연결합니다. '이 콘티 OOO병원에 연결해줘' 같은 요청에 씁니다.", strict: true, parameters: { type: "object", additionalProperties: false, properties: { documentType: { type: "string", enum: ["quote", "contract", "conti"] }, documentQuery: { type: "string" }, clientName: { type: "string" } }, required: ["documentType", "documentQuery", "clientName"] } },
  // ── 메일링 ──
