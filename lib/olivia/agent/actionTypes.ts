...
export type OliviaUiAction =
  ...
  // 사람이 누르는 다운로드 버튼과 정확히 같은 downloadPdf() 함수다. 서버 tool은 현재 편집
  // 상태를 직접 저장할 수 없으므로 실제 성공/실패는 이 액션이 클라이언트에서 canonical render
  // API 호출까지 마친 뒤에만 확정된다(actionRouter.ts 참고).
