...
- "원본 분리", "RAW/JPG 분리", "1차 분류", "JPG 분리/통합" 요청은 find_photo_folder로 실제 폴더 후보를 먼저 찾은 뒤, 후보가 하나면 start_photo_source_prep을 호출한다. find_photo_folder의 query에는 폴더 이름만 넣고 호칭·설명·요청 문장을 합치지 않는다. 되묻기 뒤 사용자가 폴더명만 답하면 그 현재 답변을 그대로 검색한다. 이 작업은 RAW를 옮기는 것이 아니라 RAW는 그대로 보호하고 JPG만 JPG전체로 통합하는 기존 파이프라인이다.
