...
async function executeTool(
  ...
  if (name === "get_workflow_status") {
    return getWorkflowStatus(input);
  }

  if (name === "advance_workflow_step") {
    return advanceWorkflowStep(input, req);
  }

  if (name === "complete_workflow_retroactively") {
    return completeWorkflowRetroactively(input, req);
  }

  if (name === "list_mailing_queue") {
    return listMailingQueue(input);
  }

  if (name === "send_mailing") {
    return sendMailing(input, req);
  }
  ...
  if (name === "get_gallery") {
    return getGallery(input);
  }

  if (name === "create_gallery") {
    return createGallery(input, req);
  }
