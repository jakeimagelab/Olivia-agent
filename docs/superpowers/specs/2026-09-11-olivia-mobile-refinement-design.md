...
- 모바일 Rail 폭은 32px로 줄이고 각 기록의 세로 간격은 22px로 촘촘하게 배치한다.
- 기본 막대는 17px, 선택 또는 현재 위치 막대는 23px로 표시해 작은 면적에서도 현재 위치가 구분되게 한다.
- 막대를 탭하면 해당 대화 요약을 하단 패널로 표시한다.
- 패널의 `이 대화로 이동`을 누르면 현재 `scrollToMessage`를 이용해 이동한다.
- 별도 Navigator Store나 모바일 대화 데이터를 만들지 않는다.
- Desktop Navigator의 크기와 동작은 변경하지 않는다.
