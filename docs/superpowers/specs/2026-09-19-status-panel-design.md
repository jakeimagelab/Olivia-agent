...
- 폴링은 하나의 타이머만 쓴다(패널 열림/닫힘과 무관하게 항상 도는 타이머 하나) — 닫혀
  있을 때는 60초 간격(버튼 경고 점만 필요), 패널이 열리면 같은 타이머를 25초 간격으로
  다시 시작하고(더 자주 볼 필요가 있으니), 닫히면 다시 60초로 되돌린다. 두 타이머가 동시에
  같은 API를 중복 호출하지 않는다. `BackupReadyNotifications`(45초)·`PhotoStudioExecutionContext`
  (10초)가 이미 쓰는 setInterval 폴링 패턴과 동일한 방식이다 — 새 폴링 메커니즘을 만들지 않는다.
