...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await db.from("agent_approvals").select("*").eq("id", params.id).single();
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await db.from("agent_approvals").update(patch).eq("id", params.id).select().single();
