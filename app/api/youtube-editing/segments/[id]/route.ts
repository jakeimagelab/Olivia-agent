...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const { data, error } = await db.from("youtube_editing_segments").update(patch).eq("id", id).select("*").maybeSingle();
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  if (!data) return NextResponse.json({ ok: false, error: "문장을 찾을 수 없습니다." }, { status: 404 });
  return NextResponse.json({ ok: true, segment: rowToSegment(data) });
}

export async function DELETE(req: NextRequest, { params }: Params) {
  const { id } = await params;
  const db = getSupabaseAdmin();
  const { error } = await db.from("youtube_editing_segments").delete().eq("id", id);
  if (error) return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
