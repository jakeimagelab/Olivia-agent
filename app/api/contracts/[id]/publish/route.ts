...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(await publishContractService(id, {
    ...
    const status = code === "NOT_FOUND" ? 404 : code === "BLOCKED" ? 400 : 500;
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "계약서 공개 실패", code }, { status });
  }
}
