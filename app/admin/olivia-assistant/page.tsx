...
export default function OliviaAssistantPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [i, a, c, e, b] = await Promise.all([
      fetch("/api/olivia/insights?limit=100", { cache: "no-store" }).then((r) => r.json()),
      fetch("/api/olivia/actions?limit=100", { cache: "no-store" }).then((r) => r.json()),
      fetch("/api/olivia/commitments?limit=100", { cache: "no-store" }).then((r) => r.json()),
      fetch("/api/olivia/events?limit=100", { cache: "no-store" }).then((r) => r.json()),
      fetch("/api/olivia/briefings/latest", { cache: "no-store" }).then((r) => r.json()),
    ]).catch(() => [] as any[]);
  ...
  useEffect(() => { load(); }, [load]);
  const customerEvents = useMemo(() => events.filter((event) => String(event.event_type).startsWith("client.")), [events]);
  const suggested = useMemo(() => actions.filter((action) => ["suggested", "prepared", "approved"].includes(action.status)), [actions]);
  const content = (() => {
  ...
  return <div className="oa-page olivia-assistant-page"><header className="olivia-page-heading"><div><span>OLIVIA PROACTIVE ASSISTANT</span><h1>능동형 AI 비서</h1><p>묻기 전에 발견하고, 말하기 전에 준비하고, 승인되면 실행합니다.</p></div><button onClick={() => fetch("/api/olivia/observer/run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mode: "all_active" }) }).then(load)}>지금 업무 확인</button></header><nav className="olivia-tabs">{TABS.map((item) => <button key={item} className={tab === item ? "is-active" : ""} onClick={() => setTab(item)}>{item}</button>)}</nav><section className="olivia-tab-content">{content}</section></div>;
}
