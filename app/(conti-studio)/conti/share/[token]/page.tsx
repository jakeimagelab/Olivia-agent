            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ background: "#155855" }}>
                  {(["#", "그룹", "장면", "장소", "시간", ...(isStaff ? ["키워드"] : []), "설명·시술", "필요인원", ...(isStaff ? ["환자역할", "준비사항"] : []), "비고"]).map((h) => (
                ...
                {sceneRows.map((scene, i) => {
                  ...
                  return (
                    <tr key={scene.id} style={{ background: i % 2 === 0 ? "#fff" : "#fafaf9" }}>
                      ...
                      {isStaff ? (
                        <td style={{ padding: "8px 10px", fontSize: 12, color: "#374151", minWidth: 160, borderBottom: "1px solid rgba(21,88,85,.07)" }}>{scene.preparation_text || "-"}</td>
                      ) : null}
