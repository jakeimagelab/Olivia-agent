...
// 메인 페이지 — 탭 전환
// ═══════════════════════════════════════════════════════════
type Tab = "queue" | "brand";

export default function MailingPage() {
  const [tab, setTab] = useState<Tab>("queue");

  const tabBtn = (t: Tab, label: string, icon: string): React.CSSProperties => ({
    height: 38, padding: "0 20px", border: "none", borderRadius: 9,
    ...
    <main style={{ height: "100vh", display: "flex", flexDirection: "column", background: C.bg, fontFamily: "'Noto Sans KR', sans-serif", color: C.txt }}>
      ...
      <header className="pc-header">
        ...
        <div style={{ display: "flex", gap: 4, background: C.mint, padding: 4, borderRadius: 12, marginLeft: "auto" }}>
          <button style={tabBtn("queue", "", "")} onClick={() => setTab("queue")}>
            📥 임시저장 메일링
          </button>
          <button style={tabBtn("brand", "", "")} onClick={() => setTab("brand")}>
            📨 브랜드 메일 보내기
          </button>
        </div>
      </header>
      ...
      {/* 탭 컨텐츠 */}
      {tab === "queue" ? <QueueTab /> : <BrandMailTab />}
    </main>
  );
}
