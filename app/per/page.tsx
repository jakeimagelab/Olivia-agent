      ...
      <header className="pc-header">
        <div className="pc-header-left">
          ...
          <div className="pc-header-brand">
            ...
            <span className="pc-header-title">PER 리워드</span>
          </div>
        </div>
        <div className="pc-header-actions">
          <button onClick={load} style={{ display:"flex", alignItems:"center", gap:6, height:36, padding:"0 14px", border:"none", borderRadius:9, background:C.light, color:C.teal, fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
      ...
      <div style={{ maxWidth:1280, margin:"0 auto", padding:"24px 20px 80px" }}>
        {loading ? (
          ...
          <>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12, marginBottom:16 }}>
              ...
              <StatCard label="총 적립 포인트"    value={`${(summary.totalEarned??0).toLocaleString()}P`} color={C.green} />
              <StatCard label="사용 가능 포인트"  value={`${(summary.totalAvailable??0).toLocaleString()}P`} color={C.orange} />
              <StatCard label="총 기부 포인트"    value={`${(summary.totalDonated??0).toLocaleString()}P`} color="#7C3AED" />
            </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
          ...
          <div style={{ background:C.white, borderRadius:14, border:`1px solid ${C.border}`, overflow:"hidden" }}>
            ...
            {recent.length === 0
              ? <div style={{ padding:32, textAlign:"center", color:C.hint, fontSize:13 }}>내역이 없습니다</div>
              : recent.map((tx:any) => (
                <div key={tx.id} style={{ padding:"12px 20px", borderBottom:`1px solid ${C.light}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  ...
                  <span style={{ fontWeight:800, fontSize:13, color:tx.points>0?C.green:C.orange }}>{tx.points>0?"+":""}{tx.points?.toLocaleString()}P</span>
                </div>
              ))
            }
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, alignContent:"start" }}>
            {NAV.map(n => (
              <Link key={n.href} href={n.href} style={{ background:C.white, borderRadius:12, border:`1px solid ${C.border}`, padding:"16px", textDecoration:"none", display:"block" }}
        ...
        {/* ESG 설명 */}
        <div style={{ marginTop:24, background:`${C.teal}08`, border:`1px solid ${C.teal}20`, borderRadius:14, padding:"20px 24px" }}>
