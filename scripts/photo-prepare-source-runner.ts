...
async function main(): Promise<void> {
  ...
  if (result.status !== "JPG_MERGE_COMPLETED") {
    process.stdout.write(`${JSON.stringify({
      ok: false,
      status: result.status === "JPG_MERGE_FAILED" ? "MERGE_FAILED" : "REVIEW_REQUIRED",
      projectPath: result.projectPath,
      jpgMoved: result.jpgMoved,
      jpgAlreadyPrepared: result.jpgAlreadyPrepared,
      rawUntouched: result.rawUntouched,
      conflicts: result.conflicts,
      error: result.conflicts.map((conflict) => `${conflict.reason}: ${conflict.source}`).join("; ") || "JPG 통합 중 충돌이 발견되었습니다.",
    })}\n`);
    process.exitCode = 2;
    return;
  }
...
main().catch((error: unknown) => {
  process.stdout.write(`${JSON.stringify({ ok: false, status: "MERGE_FAILED", error: error instanceof Error ? error.message : String(error) })}\n`);
  process.exitCode = 1;
});
